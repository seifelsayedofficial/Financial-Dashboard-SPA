...
export function kpiCard(icon, label, value, color = 'cyan') {
    ...
    <div><p class="text-xs text-slate-500 font-medium">${label}</p>
    <p class="text-lg font-bold text-slate-800 mt-0.5">${value}</p></div></div>`;
}
...
export function card(title, body, extra = '') {
    ...
    ${title ? `<div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
      <h3 class="font-bold text-slate-800">${title}</h3>${extra}</div>` : ''}
    <div class="p-6">${body}</div></div>`;
}

export function primaryBtn(text, onclick = '', icon = '', extra = '') {
    return `<button onclick="${onclick}" class="bg-gradient-to-l from-cyan-500 to-violet-500 hover:from-cyan-600 hover:to-violet-600 text-white px-5 py-2.5 rounded-xl font-semibold text-sm shadow-lg shadow-violet-500/25 hover:shadow-xl transition-all flex items-center gap-2 ${extra}">
    ${icon ? `<i data-lucide="${icon}" class="w-4 h-4"></i>` : ''}${text}</button>`;
}

export function secondaryBtn(text, onclick = '', icon = '', extra = '') {
    return `<button onclick="${onclick}" class="bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 px-4 py-2.5 rounded-xl font-medium text-sm transition-all flex items-center gap-2 ${extra}">
    ${icon ? `<i data-lucide="${icon}" class="w-4 h-4"></i>` : ''}${text}</button>`;
}

export function dangerBtn(text, onclick = '', icon = '', extra = '') {
    return `<button onclick="${onclick}" class="bg-red-50 border border-red-200 hover:bg-red-100 text-red-600 px-4 py-2.5 rounded-xl font-medium text-sm transition-all flex items-center gap-2 ${extra}">
    ${icon ? `<i data-lucide="${icon}" class="w-4 h-4"></i>` : ''}${text}</button>`;
}

export function input(label, name, type = 'text', value = '', extra = '') {
    return `<div class="space-y-1.5"><label class="block text-sm font-semibold text-slate-700">${label}</label>
...
export function textarea(label, name, value = '', rows = 3) {
    ...
    <textarea name="${name}" rows="${rows}"
      class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 outline-none transition-all resize-none">${value}</textarea></div>`;
}
...
export function emptyState(icon, msg) {
    ...
    <p class="text-lg font-medium">${msg}</p></div>`;
}
...
export function badge(text, color = 'slate') {
    ...
    return `<span class="px-2.5 py-0.5 rounded-lg text-xs font-semibold ${map[color] || map.slate}">${text}</span>`;
}
